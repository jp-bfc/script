...
customer_sheet_id = config['DEFAULT']['customer_monthly_numbers']

query = """
select
sum(rpt_net.F65) as 'Total Sales',
sum(rpt_net.F65)/sum(rpt_total.F64) as 'Average Basket Size',
sum(rpt_net.F65)/sum(rpt_net.F64) as 'Average Item Price',
sum(rpt_net.F64)/sum(rpt_total.F64) as 'Average Items Per Basket',
convert(varchar(5), DATEPART(year, rpt_net.F254)) + ' ' + convert(varchar(10), DATENAME(month, DATEADD(month, DATEPART(month, rpt_net.F254), -1))) as 'Month',
clg.F1268 as 'Shareholder Type', sum(rpt_total.F64) as 'Total Transactions'
from (select * from STORESQL.dbo.RPT_CLT_M where F1034 = 3) rpt_net
inner join (select * from STORESQL.dbo.RPT_CLT_M where F1034 = 2) rpt_total on rpt_net.F254 = rpt_total.F254 and rpt_net.F1148 = rpt_total.F1148
inner join STORESQL.dbo.TLZ_TAB t on rpt_net.F1034 = t.F1034
inner join STORESQL.dbo.CLT_TAB clt on rpt_net.F1148 = clt.F1148
inner join STORESQL.dbo.CLG_TAB clg on clt.F1154 = clg.F1154
inner join STORESQL.dbo.CLF_TAB clf on clt.F1152 = clf.F1152
group by rpt_net.F254, rpt_net.F1034, t.F1128, t.F1129, t.F1039, clg.F1268
order by rpt_net.F254 desc, rpt_net.F1034, clg.F1268
"""

customer_frame = pd.read_sql(query, cnxn)

month_list = customer_frame['Month'].unique().tolist()
format_updates = []
customer_frame = customer_frame[['Month','Shareholder Type', 'Total Transactions', 'Total Sales',  'Average Basket Size', 'Average Item Price', 'Average Items Per Basket']]
customer_frame = customer_frame.astype(str)

pivoted_frame = pd.pivot_table(customer_frame, values = ['Total Transactions', 'Total Sales',  'Average Basket Size', 'Average Item Price', 'Average Items Per Basket'], index=['Shareholder Type'], columns =['Month'] ).reset_index()
pivoted_frame.fillna('', inplace=True)
pivoted_frame = pivoted_frame.stack().set_index(['Shareholder Type']).reset_index()
print(pivoted_frame)
pivoted_frame.fillna("", inplace=True)
sheet = batchUpdate(customer_sheet_id, addSheet(sheetProp(title="Overview", gridProperties=gridProp(pivoted_frame.shape[0]+1, pivoted_frame.shape[1], 1))))
sheet_id = sheet.get('replies', '')[0].get('addSheet', '').get('properties', '').get('sheetId', '')
append(customer_sheet_id, "Overview!A1:B1", "ROWS", [pivoted_frame.columns.tolist()], "USER_ENTERED")
append(customer_sheet_id, "Overview!A1:B1", "ROWS", pivoted_frame.to_numpy().tolist(), "USER_ENTERED")
format_updates.extend([
    repeatCell(
        gridRange(sheet_id, 0, 1, 0, pivoted_frame.shape[1]),
    ...
    repeatCell(
        gridRange(sheet_id, 1, pivoted_frame.shape[0]+1, 0, pivoted_frame.shape[1]),
        cellData(userEnteredFormat=cellFormat(
                                    borders=borders(*all_solid),
                                    textFormat=textFormat(fontFamily="Arial", fontSize=12)
                                ))),
    repeatCell(
            gridRange(sheet_id, 1, pivoted_frame.shape[0]+1, 2, 5),
            cellData(userEnteredFormat=cellFormat(numberFormat=currency))
            ),
    repeatCell(
            gridRange(sheet_id, 1, pivoted_frame.shape[0]+1, 5, 6),
            cellData(userEnteredFormat=cellFormat(numberFormat=numberFormat("NUMBER", "0.0")))
            ),
    veryBasicFilter(gridRange(sheet_id, 0, pivoted_frame.shape[0]+1, 0, pivoted_frame.shape[1])),
    autoSize(sheet_id, "COLUMNS"),
])
batchUpdate(customer_sheet_id, format_updates)

#print(customer_frame)
# for m in month_list:
#     print(m)
#     month_frame = customer_frame[customer_frame['Month'] == m]
#     month_frame = month_frame.drop(['Month'], axis=1)
#     month_frame.fillna("", inplace=True)
    
#     sheet = batchUpdate(customer_sheet_id, addSheet(sheetProp(title=m, gridProperties=gridProp(month_frame.shape[0]+1, month_frame.shape[1], 1))))
#     sheet_id = sheet.get('replies', '')[0].get('addSheet', '').get('properties', '').get('sheetId', '')
#     append(customer_sheet_id, m+"!A1:B1", "ROWS", [month_frame.columns.tolist()], "USER_ENTERED")
#     append(customer_sheet_id, m+"!A1:B1", "ROWS", month_frame.to_numpy().tolist(), "USER_ENTERED")
#     format_updates.extend([
#         repeatCell(
#             gridRange(sheet_id, 0, 1, 0, month_frame.shape[1]),
#             cellData(userEnteredFormat=cellFormat(
#                                         borders=borders(*all_thick),
#                                         horizontalAlignment="CENTER",
#                                         verticalAlignment="MIDDLE",
...
#                                     ))),
#         repeatCell(
#             gridRange(sheet_id, 1, month_frame.shape[0]+1, 0, month_frame.shape[1]),
#             cellData(userEnteredFormat=cellFormat(
#                                         borders=borders(*all_solid),
#                                         textFormat=textFormat(fontFamily="Arial", fontSize=12)
#                                     ))),
#         repeatCell(
#                 gridRange(sheet_id, 1, month_frame.shape[0]+1, 2, 5),
#                 cellData(userEnteredFormat=cellFormat(numberFormat=currency))
#                 ),
#         repeatCell(
#                 gridRange(sheet_id, 1, month_frame.shape[0]+1, 5, 6),
#                 cellData(userEnteredFormat=cellFormat(numberFormat=numberFormat("NUMBER", "0.0")))
#                 ),
#         veryBasicFilter(gridRange(sheet_id, 0, month_frame.shape[0]+1, 0, month_frame.shape[1])),
#         autoSize(sheet_id, "COLUMNS"),
#     ])
#     time.sleep(2)
# batchUpdate(customer_sheet_id, format_updates)