...
# with open(r"\\bfc-hv-01\SWAP\Transaction Logs By Month\New folder\\TRANSACTIONS-SEPTEMBER-2022.txt") as f:
#     for _ in range(4):
#         next(f)
#     while f:
#         line = f.readline()
...
items = pd.read_sql("""
SELECT obj.F01 as 'UPC', obj.F29 as 'Desc', rpc.F1024 as 'Dept', sdp.F1022 as 'Sub-Dept'
...
#print(items)
#print(df)
joined = items.merge(df, how='left', on=['UPC'])

#joined['Name'] = joined['UPC'] + " " + joined['Desc']
#print("Joined", joined)
# dept_list = joined['Dept'].unique().tolist()
# for dept in dept_list:
#     print(dept)
#     dept_frame = joined[joined['Dept'] == dept]
#     dept_frame = dept_frame.drop(['Dept'], axis=1)
#     dept_frame['Name'] = dept_frame['UPC'] + dept_frame['Desc'] + dept_frame['Sub-Dept']
#     analysis = dept_frame.groupby(['Trans', 'Name'])['Quantity'].sum().unstack().reset_index().fillna(0).set_index("Trans")

#     encoded = analysis.applymap(encode_units)
#     # print(encoded.shape[0])
#     # filtered_items = encoded[(encoded > 0).sum(axis=1) >= 2]
#     # print(filtered_items.shape[0])
#     freq_items = apriori(encoded, min_support=0.0075, use_colnames=True).sort_values('support', ascending=False).reset_index(drop=True)
#     if freq_items.shape[0] > 0:
#         freq_items.to_csv("\\\\bfc-hv-01\\SWAP\\PythonScripts\\Basket Analyses\\%s_freq_items.csv" % dept)
#         batchUpdate(cashback_report_id, addSheet(sheetProp(title=(dept + " Freq"), gridProperties=gridProp(freq_items.shape[0]+1, freq_items.shape[1], 1))))
#         freqId = [x for x in sheets.spreadsheets().get(spreadsheetId=cashback_report_id).execute().get('sheets', '')
#                     if x.get('properties', '').get('title', '') == (dept + " Freq")][0].get('properties', '').get('sheetId', '')
#         append(cashback_report_id, (dept + " Freq"), "ROWS", [freq_items.columns.tolist()], "RAW")
#         append(cashback_report_id, (dept + " Freq"), "ROWS", freq_items.astype(str).to_numpy().tolist(), "RAW")
#         #print(freq_items)
#         associations = association_rules(freq_items, metric='lift', min_threshold=1).sort_values('lift', ascending=False).reset_index(drop=True)
#         associations = associations[~associations[['antecedents', 'consequents']].apply(frozenset, axis=1).duplicated()].reset_index(drop=True)
#         if associations.shape[0] > 0:
#             associations.to_csv("\\\\bfc-hv-01\\SWAP\\PythonScripts\\Basket Analyses\\%s_finished.csv" % dept)
#             batchUpdate(cashback_report_id, addSheet(sheetProp(title=(dept + " Associations"), gridProperties=gridProp(associations.shape[0]+1, associations.shape[1], 1))))
#             associationId = [x for x in sheets.spreadsheets().get(spreadsheetId=cashback_report_id).execute().get('sheets', '')
#                     if x.get('properties', '').get('title', '') == (dept + " Associations")][0].get('properties', '').get('sheetId', '')
#             append(cashback_report_id, (dept + " Associations"), "ROWS", [associations.columns.tolist()], "RAW")
#             append(cashback_report_id, (dept + " Associations"), "ROWS", associations.astype(str).to_numpy().tolist(), "RAW")
    
    
banana_test = joined[joined['UPC'] == '0000000094011']
print(banana_test['Quantity'].sum())

joined['Name'] = joined['UPC'] + joined['Desc'] + joined['Sub-Dept']
analysis = joined.groupby(['Trans', 'Name'])['Quantity'].sum().unstack().reset_index().fillna(0).set_index("Trans")
print(analysis)
encoded = analysis.applymap(encode_units)
# print(encoded.shape[0])
# filtered_items = encoded[(encoded > 0).sum(axis=1) >= 2]
# print(filtered_items.shape[0])
freq_items = apriori(encoded, min_support=0.02, use_colnames=True).sort_values('support', ascending=False).reset_index(drop=True)
if freq_items.shape[0] > 0:
    freq_items.to_csv("\\\\bfc-hv-01\\SWAP\\PythonScripts\\Basket Analyses\\%s_freq_items.csv" % "All")
    batchUpdate(cashback_report_id, addSheet(sheetProp(title=("All" + " Freq"), gridProperties=gridProp(freq_items.shape[0]+1, freq_items.shape[1], 1))))
    freqId = [x for x in sheets.spreadsheets().get(spreadsheetId=cashback_report_id).execute().get('sheets', '')
                if x.get('properties', '').get('title', '') == ("All" + " Freq")][0].get('properties', '').get('sheetId', '')
    append(cashback_report_id, ("All" + " Freq"), "ROWS", [freq_items.columns.tolist()], "RAW")
    append(cashback_report_id, ("All" + " Freq"), "ROWS", freq_items.astype(str).to_numpy().tolist(), "RAW")
    #print(freq_items)
    associations = association_rules(freq_items, metric='lift', min_threshold=1).sort_values(['support'], ascending=False).reset_index(drop=True)
    associations = associations[~associations[['antecedents', 'consequents']].apply(frozenset, axis=1).duplicated()].reset_index(drop=True)
    if associations.shape[0] > 0:
        associations.to_csv("\\\\bfc-hv-01\\SWAP\\PythonScripts\\Basket Analyses\\%s_finished.csv" % "All")
        batchUpdate(cashback_report_id, addSheet(sheetProp(title=("All" + " Associations"), gridProperties=gridProp(associations.shape[0]+1, associations.shape[1], 1))))
        associationId = [x for x in sheets.spreadsheets().get(spreadsheetId=cashback_report_id).execute().get('sheets', '')
                if x.get('properties', '').get('title', '') == ("All" + " Associations")][0].get('properties', '').get('sheetId', '')
        append(cashback_report_id, ("All" + " Associations"), "ROWS", [associations.columns.tolist()], "RAW")
        append(cashback_report_id, ("All" + " Associations"), "ROWS", associations.astype(str).to_numpy().tolist(), "RAW")
#print("\n********************\n", associations)
    # for code in [
    #     # '0000000000052',
    #     # '0000000000055',
