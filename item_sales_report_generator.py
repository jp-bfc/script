...
#convert(varchar(5), DATEPART(year, rpt.F254)) + ' ' + convert(varchar(10), DATENAME(month, DATEADD(month, DATEPART(month, rpt.F254), -1))) as 'Month',
query = """
select
rpt.F01 as 'UPC', sum(rpt.F64) as 'Volume', sum(rpt.F65) as 'Sales', obj.F29 as 'Name', rpc.F1024 as 'Dept'
...
where
rpt.F254 in ('2022-10-31 00:00:00.000','2022-09-30 00:00:00.000','2022-08-31 00:00:00.000', '2022-07-30 00:00:00.000')
and rpt.F01 in (
'0000000008478',
'0000000008479',
'0000000008480'
)
group by rpt.F01, obj.F29, rpc.F1024
"""

sales_frame = pd.read_sql(query, cnxn)
if pivot:
    pivoted_frame = pd.pivot_table(sales_frame, values = ['Sales', 'Volume'], index=['UPC', 'Name', 'Dept'], columns = ['Month']).reset_index()
    pivoted_frame.fillna('', inplace=True)
    report_book = createBook(bookName, sales_reports_folder, 'application/vnd.google-apps.spreadsheet', drive)
    report_book_id = report_book.get('id', '')
    batchUpdate(report_book_id, addSheet(sheetProp(title=sheetName, gridProperties=gridProp(pivoted_frame.shape[0]+1, pivoted_frame.shape[1], 1))))
    append(report_book_id, sheetName + "!A1:B1", "ROWS", pivoted_frame.columns.tolist(), "RAW")
    append(report_book_id, sheetName + "!A1:B1", "ROWS", pivoted_frame.to_numpy().tolist(), "RAW")
else:
    sales_frame.fillna('', inplace=True)
    report_book = createBook(bookName, sales_reports_folder, 'application/vnd.google-apps.spreadsheet', drive)
    report_book_id = report_book.get('id', '')
    batchUpdate(report_book_id, addSheet(sheetProp(title=sheetName, gridProperties=gridProp(sales_frame.shape[0]+1, sales_frame.shape[1], 1))))
    append(report_book_id, sheetName + "!A1:B1", "ROWS", [sales_frame.columns.tolist()], "RAW")
    append(report_book_id, sheetName + "!A1:B1", "ROWS", sales_frame.to_numpy().tolist(), "RAW")