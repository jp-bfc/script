...
sales_reports_folder = config['DEFAULT']['sales_report_folder']
coupon_reports_book = config['DEFAULT']['coupon_report_sheet']

##COUPON QUERY OF SALES HISTORY
query = """
select
CONVERT(date, rpt.F254) as [Month], tlz.F1039 as [Coupon Type],
rpt.F01 as [Item Code/UPC], rpt.F64 as [Quantity], rpt.F65 as [Discount/Revenue],
obj.F18 as [Dept Code], obj.F29 as [Name], obj.F155 as [Brand], sdp.F04 as [Sub Dept Code], pos.F383 as [Coupon PID]
from STORESQL.dbo.RPT_ITM_M rpt
inner join STORESQL.dbo.OBJ_TAB obj on rpt.F01 = obj.F01
left join STORESQL.dbo.POS_TAB pos on pos.F01 = obj.F01
inner join STORESQL.dbo.SDP_TAB sdp on pos.F04 = sdp.F04
inner join STORESQL.dbo.TLZ_TAB tlz on rpt.F1034 = tlz.F1034
where sdp.F04 = 900 and rpt.F1034 <> 33
order by rpt.F254 desc
"""

report_frame = pd.read_sql(query, cnxn)
report_frame = report_frame.applymap(str)
pivoted_frame = pd.pivot_table(
    report_frame,
    values = ['Quantity', 'Discount/Revenue'],
    index=['Item Code/UPC', 'Name','Coupon Type'],
    columns =['Month'])

# for x in pivoted_frame:
#     print(pivoted_frame[x])
pivoted_frame.fillna("", inplace=True)
print(pivoted_frame)
#pivoted_frame = pivoted_frame.applymap(str)
# print(pivoted_frame)
# print(pivoted_frame.dtypes)
# z = zip(pivoted_frame.index.values,pivoted_frame['Quantity', 'Discount/Revenue'].values)
ls = []
for y in pivoted_frame.index.values:
    print(y)
    item = y[0]
    name = y[1]
    t = y[2]
    values = [list(a) for a in zip(pivoted_frame.loc[y]['Quantity'], pivoted_frame.loc[y]['Discount/Revenue'])]
    values.reverse()
    val = [i for sub in values for i in sub]
    val.insert(0, y[2])
    val.insert(0, y[1])
    val.insert(0, y[0])
    ls.append(val)
...
"",
'2020-07-31'],
["Item Code/UPC", "Name", "Coupon Type",
"Volume",
'Discount/Revenue', 
"Volume",
'Discount/Revenue', 
...
batchUpdate(coupon_reports_book, addSheet(sheetProp(title="Historical3", gridProperties=gridProp(pivoted_frame.shape[0]+1, pivoted_frame.shape[1], 2))))
append(coupon_reports_book, "Historical3!A1:B1", "ROWS", headers, "USER_ENTERED")
append(coupon_reports_book, "Historical3!A3:B3", "ROWS", ls, "USER_ENTERED")

#print([pivoted_frame[x].values for x in pivoted_frame])
# pivoted_frame.columns = pivoted_frame.columns.map(' : '.join)
# print(pivoted_frame)
# print(pivoted_frame.to_numpy().tolist())
# print(pivoted_frame.columns.names)
#pivoted_frame.to_csv("pivot.csv")