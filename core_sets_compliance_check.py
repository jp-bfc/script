...
core_compliance_folder = config['DEFAULT']['core_compliance_folder']
...
core_frame = pd.read_csv("Core Sets - Consolidated CS Cost Supports (1) (1).csv", converters={'UPC':str})

merged = core_frame.merge(sms_frame, how="inner", on='UPC')

merged = merged[merged['Core Price'] != merged['SMS Price']]
merged['Diff'] = merged['Core Price'] - merged['SMS Price']
merged['Change Price'] = ""
merged.fillna("", inplace=True)
merged = merged.astype(str)


period_name = 'October 2022'
...
dept_list = merged['Dept'].unique().tolist()
batchUpdate(compliance_id, addSheet(sheetProp(title="SMS Import", gridProperties=gridProp(merged.shape[0]+1, merged.shape[1], 1))))
append(compliance_id, "SMS IMPORT!A1:B1", "ROWS", [merged.columns.tolist()], "RAW")
append(compliance_id, "SMS IMPORT!A2:B2", "ROWS", merged.to_numpy().tolist(), "RAW")

format_updates = []
for dept in dept_list:
    dept_frame = merged[merged['Dept'] == dept]
